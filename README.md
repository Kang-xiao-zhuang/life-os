...
> 装签名版之前先看一眼 `CLAUDE.md` 的 *Signed release build*：签名密钥丢了就没法再覆盖安装，
> 只能卸载重装 —— 那会连数据一起删掉。
...
**今日最重要不是卡片** —— 它是首页唯一有强调的地方：小标签 + 大一号的任务行，直接坐在背景上；
其余四张卡标题降一级、颜色降一级。以前五张卡片长得一模一样，眼睛不知道该先看哪儿。
...
- 「今天」旁边有**每日一句**，从日期挑，当天不变、第二天换一条。
  不是每次刷新都随机：那样会在你勾任务时闪来闪去，也就不像「今天的一句」了。
  语气按 App 自己的调子写（「一次只做一件事」这种），喊口号的话第三个早上就烦了
...
- **重复任务** —— 每天 / 每周 / 每月三档。勾掉之后自动生成下一条，日期锚在原来的截止日上，
  所以「每周一交周报」晚一天勾也不会漂到周二。
  取消勾选会把刚生成的那条**收回**：勾错再点回来是最普通的操作，不该在未来留下一条重复的。
  下一条不继承「今日最重要」—— 那是你为某一天做的决定，不是任务本身的属性
- 每行会标出「每周」这类标记，因为重复任务和一次性任务长得一样，而「这条会自己回来」
  正是你决定今天要不要硬做完它之前想知道的
...
> ⚠️ **v1.1.0 起数据库是 schema v2**（重复任务加了一列）。`config.json` 里记着 schema 版本，
> 版本不一致会被**拒绝导入**，所以 v1.0.0 导出的备份在 v1.1.0 里导不回来了。
> 升级后请重新导出一份。这是知情选择：为让旧备份可用而写兼容层，不值得。
...
**评估后没做**
- **天气** —— 2026-08-06 第二次评估后确认不做。理由不是「多一个权限」：
  - `POST_NOTIFICATIONS` 让 App 能跟**你**说话，`INTERNET` 让它能跟**任何人**说话。
    加了之后，「本地优先」就从**清单文件强制保证的事实**，变成一句需要你信任的承诺
  - 它会是唯一一个**会因为外部原因坏掉**的功能。其他每个功能在飞机上都能用、永远不会因为
    别人的服务器出问题而失灵；一个本地优先的工作台里放一个会自己坏的格子，代价比一行权限大
  - 手机锁屏、通知栏、启动器的 At a Glance 本来就都有天气 —— **看见 ≠ 用到**
  - （无 key 的 open-meteo 实测可达，约 1 秒。真要加，就手动输城市名而不用定位，
    不填就一次网都不联，失败就不显示；但先说服自己上面三条）
- **日出日落**（不联网那种）—— 提过没做。从纬度 + 日期用闭式公式本地算，零依赖零权限，
  「今晚还能不能去跑步」这件事它答得不比天气差。想要的话随时能加
- **自动本地备份** —— 有价值，暂时搁置
